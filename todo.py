...
def index():
    db, c = get_db()
    c.execute(
        'select t.id, t.description, u.username, t.completed, t.create_at from todo t JOIN user u on t.created_by = u.id where t.created_by = %s order by create_at desc',
        (g.user['id'],)
    )
    todos = c.fetchall()
...
def get_todo(id):
    db, c = get_db()
    c.execute(
        'select t.id, t.description, t.completed, created_by, t.create_at, u.username '
